...
# Global settings
my $terms = "\"Arch%20Ophthalmol\"[Journal]+AND+has+abstract[filter]";
#my $terms = "AMD";
#$terms = "\"JAMA\"[Journal]+AND+has+abstract[filter]";
#$terms = "\"Lancet\"[Journal]+AND+has+abstract[filter]";
my $OUTFile = "archivesOphtho.xml";
...
fetchNCBI($first, "http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&retmax=200&usehistory=y&email=aaron\@cogent14.com&term=$terms");
my $data = $xml->XMLin($first);
my $querykey = $data->{QueryKey};
my $webenv = $data->{WebEnv};
fetchNCBI($second, "http://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&retmode=xml&query_key=$querykey&rettype=full&WebEnv=$webenv&email=aaron\@cogent14.com&retstart=0&retmax=100");
...
foreach my $article (@{$data2->{PubmedArticle}}) {
    ...
    if (ref($abstract) eq "ARRAY") {
        foreach my $subabstract (@{$abstract}) {
            $trueabstract .= "\n".$subabstract->{Label}.": ".$subabstract->{content}." ";
        }
    ...
    my $simplink;
    if (ref($linkxml->{LinkSet}->{IdUrlList}->{IdUrlSet}->{ObjUrl}) eq "ARRAY") {
        foreach my $linkone (@{$linkxml->{LinkSet}->{IdUrlList}->{IdUrlSet}->{ObjUrl}}) {
            $simplink = $linkone->{Url};
        }
    } else {
