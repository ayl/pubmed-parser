...
# Global settings
my $terms = "\"Proc Natl Acad Sci U S A\"[Journal]+AND+has+abstract[filter]";
my $retmax = 50;
#my $terms = "AMD";
...
fetchNCBI($first, "http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&retmax=100&sort=pub+date&usehistory=y&email=aaron\@cogent14.com&term=$terms");
my $data = $xml->XMLin($first);
my $querykey = $data->{QueryKey};
my $webenv = $data->{WebEnv};
fetchNCBI($second, "http://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&retmode=xml&query_key=$querykey&rettype=full&WebEnv=$webenv&email=aaron\@cogent14.com&retstart=0&retmax=$retmax");
